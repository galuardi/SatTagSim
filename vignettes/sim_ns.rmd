...
myworld =  rworldmap::getMap(resolution  =  'high') %>% sf::st_as_sf()
...
knitr::kable(simvals, caption  =  "Control parameters for simulation")
...
knitr::kable(morder, caption  =  'Month order for multi-month start simulation', row.names  =  T)
```
...
ggplot()+
  ...
  # geom_sf(box11sf, mapping = aes(), col = 'salmon', fill = 'transparent', size = 1)+
  geom_sf(myworld, mapping = aes(), fill = 'grey90', col = 'grey50')+
  geom_point(data = spts[[2]], mapping = aes(x, y), size = .4, ,col = 'orangered')+
...
plot.boxtrans(boxtrans, text.size = 2.5, strip.text = 12)
```
