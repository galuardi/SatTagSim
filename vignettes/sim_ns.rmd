...
make.tag.raster  <- function(simdat, xmn  =  -100, xmx  =  30, ymn  =  0, ymx  =  60, boxsize =  60){
  # require(raster)
  if(class(simdat)[1] =="SpatialPointsDataFrame"){
...
sstdata2 = sstdata2 |> apply(3, FUN = function(x) {raster(x) |> t() |> flip()}) |> stack()
...
cl  =  makeCluster(ncores, type = 'FORK') # if on Linux, use the FORK!
...
plot.boxtrans(boxtrans, text.size = 2.5, strip.text = 12, axis.text = 12)
```
