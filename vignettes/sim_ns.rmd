...
```{r load pre-simulated dataset, eval = T, echo = F}
...
```{r plot_seas_change, eval  =  F, fig.cap  =  'Seasonal shifts (transitions) expressed spatially. 7-box model used to illustrate how spatial results may differ from results expressed according to stratification.'}
sr  =  lapply(split(simdatdf, simdatdf$seas), function(x) make.sim.raster(x, boxsize  =  150))
...
```{r plot_seas_change_month, eval  =  F, fig.cap  =  'Monthly shifts (transitions) expressed spatially.  7-box model used to illustrate how spatial results may differ from results expressed according to stratification.'}
sr  =  lapply(split(simdatdf, simdatdf$Month), function(x) make.sim.raster(x, boxsize  =  150))
