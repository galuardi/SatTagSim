...
npmon = 1 # number of simulation steps per month.
...
for(i in 1:12){
  ...
  # test = make.sim.track.par(tpar = simpar, morder = rep(simorder, nyears), sp = sp, bath = bath, sstmat = sstmat, seaslen = npmon, sstol = sstol, mcoptions = mcoptions)
    test = make.sim.track.par2(par_array = par_array, boxmat = boxmat, simorder = rep(simorder, nyears), sp = sp, bath = bath, sstmat = sstmat, seaslen = npmon, sstol = sstol, mcoptions = mcoptions)
  simdat[[i]] = test
...
for(i in 1:12){
  ...
  lapply(k, function(j) points(simdat[[j]][1,1:2], col = hsv(.99, .9, .9, alpha = .5), pch = 19, cex = 1))
  k = k+50
}
